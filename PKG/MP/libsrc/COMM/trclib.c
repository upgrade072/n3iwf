...
int		trcFlag=0, trcLogFlag=0, trcOmdFlag=0;
...
int trclib_printOut (char *fName, int lNum, char *msg)
{
	...
	if ((len = fprintf (trcFp, "[%s.%03d] %s", tmp, (int)(curr.tv_usec/1000), msg)) < 0) {
		trcFlag = 0;
		trcFp = NULL;
	...
	if ((curr.tv_sec - trcStartTime) >= TRCLIB_MAX_TRC_TIMER) {
        logPrint (trcLogId,FL,"  TRACE LOG OFF AUTOMATICALLY (due to time limit)\n\n");
		if (trcFp != NULL) {
...
int trclib_writeLog (char *fName, int lNum, char *msg)
{
	...
	if (trcLogFlag) {
		...
		if ((now - trcLogStartTime) >= TRCLIB_MAX_TRC_TIMER) {
			trcLogFlag = 0;
            logPrint (trcLogId,FL,"  MSG_LOG OFF AUTOMATICALLY (due to time limit)\n\n");
			if (trcFp != NULL) {
